                ...
                <tbody>
                    @foreach ($pinjaman->pinjamans as $nestedItem)
                        <tr>
                            ...
                            @if ($nestedItem->status_pinjaman_d == 0)
                                <td>
                                    <form method="POST" action="/admin/detailUser/bayarPinjaman/{{$nestedItem->id_pinjaman_d}}">
                                        @csrf
                                        <input type="hidden" name="nominal" value="{{$nestedItem->pinjaman}}">
                                        <button type="submit" class="buttonEdit btn btn-success">
                                            <span>Bayar</span>
                                        </button>
                                    </form>
                                </td>
                            @else
                                <td>
                                    ...
                                    <form method="POST" action="/admin/detailUser/batalkanPembayaran/{{$nestedItem->id_pinjaman_d}}">
                                        @csrf
                                        <input type="hidden" name="nominal" value="{{$nestedItem->pinjaman}}">
                                        <button type="submit" class="buttonEdit btn btn-danger">
                                            <span>Batal</span>
                                        </button>
                                    </form>
                                </td>
